    ...
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-6 py-24">
          ...
          <p className="mt-4 text-muted-foreground">
            We offer a full stack of services to build, deploy, and grow AI-powered
            products and digital experiences.
          </p>

          <section className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((s, i) => (
              <div key={i} className="card glass p-6 hover:shadow-lg">
                <h3 className="font-semibold text-lg">{s.title}</h3>
                <p className="mt-2 text-sm text-muted-foreground">{s.desc}</p>
                <Link to="/contact" className="mt-4 inline-block text-primary">
                  Talk to us →
                </Link>
              </div>
            ))}
